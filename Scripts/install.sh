...
enter_tty_mode() {
    if [ "$TTY_MODE_ACTIVE" -eq 1 ]; then
        return 0
    fi

    if tty -s; then
        TTY_STATE_SNAPSHOT="$(stty -g 2>/dev/null || true)"
        printf '\e[?25l'
        stty -echo
        TTY_MODE_ACTIVE=1
    fi
...
restore_tty_mode() {
    if [ "$TTY_MODE_ACTIVE" -eq 1 ]; then
        if [ -n "$TTY_STATE_SNAPSHOT" ]; then
            stty "$TTY_STATE_SNAPSHOT" 2>/dev/null || stty echo 2>/dev/null || true
        else
...
parse_arguments() {
    ...
    local package_count
    package_count=$(
        find "$installers_path" \
            \( \
                \( -type f \( -name "*.dmg" -o -name "*.iso" -o -name "*.pkg" -o -name "*.zip" \) \) -o \
                \( -type d -name "*.app" \) \
            \) \
            ! -name "._*" ! -name ".DS_Store" | wc -l | tr -d ' '
    )
...
analyze_local_packages() {
    ...
    local temp_file
    temp_file="$(mktemp -t installflow_packages_list.XXXXXX)"
    if [ -z "$temp_file" ]; then
        error "无法创建临时文件"
        return 1
    fi
    ...
    find "$LOCAL_INSTALLERS_DIR" \
        \( \
            \( -type f \( -name "*.dmg" -o -name "*.iso" -o -name "*.pkg" -o -name "*.zip" \) \) -o \
            \( -type d -name "*.app" \) \
        \) \
        ! -name "._*" ! -name ".DS_Store" > "$temp_file"
...
install_zip_file() {
    ...
    local temp_dir
    temp_dir="$(mktemp -d -t installflow_zip.XXXXXX)"
    if [ -z "$temp_dir" ]; then
        failed_installs+=("$filename (无法创建ZIP临时目录)")
        return 1
    fi
